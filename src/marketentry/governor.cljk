(ns marketentry.governor
  ...
  notion of jurisdictional procurement law, whether an Ireland-resident
  ...
  actuation is always a human call.
    ...
    2. Evidence incomplete         -- for `:filing/draft`/
                                       ...
                                       checklist on file?
    3. Ireland-resident-rep missing  -- for `:filing/submit`, when the
                                       engagement declares
                                       `:requires-eu-establishment?
                                       true` (near-universal for Ireland
                                       ...
                                       Grounded in EU establishment or Irish
                                       CRO entity for many procedures
                                       (contracting authorities / OGP).
    ...
    (let [e (store/engagement st subject)
      ...
      (when-not (and assessment
        ...
        [{:rule :evidence-incomplete
          :detail "法域の必要書類(CRO/統一資格/eTenders登録/代理人確認等)が充足していない状態での提案"}]))))
...
(defn- eu-establishment-missing-violations
  ...
  `:requires-eu-establishment?` ground truth (most Ireland public
    ...
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-eu-establishment? e))
        ...
        [{:rule :eu-establishment-missing
          :detail (str subject " はアイルランド居住代理人を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn- cro-unverified-violations
  "For `:filing/submit`, when the engagement declares
  `:requires-cro? true`, INDEPENDENTLY check
  `:cro-verified?` -- CONDITIONAL on the engagement's own
  ground truth."
  [{:keys [op subject]} st]
  (when (= op :filing/submit)
    (let [e (store/engagement st subject)]
      (when (and (true? (:requires-cro? e))
                 (not (true? (:cro-verified? e))))
        [{:rule :cro-unverified
          :detail (str subject " はCRO確認を要するが未確認 -- 提出提案は進められない")}]))))
...
(defn check
  ...
  (let [hard (into []
                   (concat (spec-basis-violations request proposal)
                           ...
                           (cro-unverified-violations request st)
