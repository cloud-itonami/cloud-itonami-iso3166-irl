...
2. Register the client's intake: business type, target public function,
   prior filing history in Ireland if any.
3. Run the advisor in read-only mode against eTenders, Ireland's official public procurement platform.
...
- named referral relationship with Irish-licensed counsel or a
...
a working referral relationship with Irish-licensed counsel or a
registered agent for whatever licensed representation the law of
Ireland requires for actual public-procurement filings.
