(ns marketentry.store
  "SSoT for the IRL market-entry compliance actor, behind a `Store`
...
(defn demo-data
  ...
  []
  {:engagements
   {"eng-1" {:id "eng-1" :operator "Kita Systems IE" :portal "eTenders"
             ...
             :requires-cro? true :cro-verified? true
             :drafted? false :submitted? false
             :jurisdiction "IRL" :status :intake}
    "eng-2" {:id "eng-2" :operator "Atlantis LLC" :portal "eTenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-cro? true :cro-verified? true
    ...
    "eng-3" {:id "eng-3" :operator "Minami Systems" :portal "eTenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             ...
             :drafted? false :submitted? false
             :jurisdiction "IRL" :status :intake}
    "eng-4" {:id "eng-4" :operator "Higashi Export" :portal "eTenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             ...
             :drafted? false :submitted? false
             :jurisdiction "IRL" :status :intake}
    "eng-5" {:id "eng-5" :operator "Nishi Logistics" :portal "eTenders"
             :base-fee 500000 :monthly-rate 30000 :monitoring-months 12
             :claimed-fee 860000.0
             :requires-eu-establishment? true :has-eu-establishment? true
             :requires-cro? true :cro-verified? false
             :drafted? false :submitted? false
             :jurisdiction "IRL" :status :intake}
    "eng-6" {:id "eng-6" :operator "Chuo Civic Tech" :portal "eTenders"
...
(defn- engagement->tx [{:keys [id operator portal base-fee monthly-rate monitoring-months claimed-fee
  ...
  (cond-> {:engagement/id id}
    ...
    (some? requires-cro?)    (assoc :engagement/requires-cro? requires-cro?)
    (some? cro-verified?)    (assoc :engagement/cro-verified? cro-verified?)
...
(def ^:private engagement-pull
  [:engagement/id :engagement/operator :engagement/portal :engagement/base-fee :engagement/monthly-rate
   ...
   :engagement/requires-cro? :engagement/cro-verified?
...
(defn- pull->engagement [m]
  (when (:engagement/id m)
    {:id (:engagement/id m) :operator (:engagement/operator m) :portal (:engagement/portal m)
     ...
     :requires-cro? (boolean (:engagement/requires-cro? m))
     :cro-verified? (boolean (:engagement/cro-verified? m))
