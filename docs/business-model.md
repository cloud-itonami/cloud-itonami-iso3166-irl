# Business Model: Independent Public-Sector Market-Entry & Procurement Compliance Service — Ireland

## Classification

- Repository: `cloud-itonami-iso3166-irl`
- ISO 3166: `IRL` (Ireland)
...
- an already-incorporated `cloud-itonami-cofog-{code}` /
  ...
  `cloud-itonami-{ISIC}` operator wanting to bid on an Irish
  public contract
...
- registration walkthrough for eTenders, Ireland's official public procurement platform
- business/tax registration checklist: a CRO (Companies Registration Office) company registration; VAT registration with Revenue where the taxable-turnover threshold is met
- local-content / preferential-procurement navigation: EU-wide open tendering above EU thresholds (no national-content quota as an EU member state), but a domestic/EU address for service is typically required
...
- this service does **not** provide legal or tax advice; characterization
  ...
  routes to Irish-licensed counsel or a registered agent
...
- **`cloud-itonami-cofog-{code}`**: a jurisdiction-agnostic operator
  ...
  blueprint AND this one to operate in Ireland).
